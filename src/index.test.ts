        ...
        for (const inputType of inputTypes) {
          describe(`input: ${inputType.description}`, () => {
            test("computes the autocorrelation of small datasets", () => {
              ...
              expect(
                autocorrelate(inputType.arrayConverter([1, 2, 1]))
              ).toBeDeepCloseTo([6, 4, 1], 6);

              expect(
                autocorrelate(inputType.arrayConverter([1, 0, 1, 0]))
              ).toBeDeepCloseTo([2, 0, 1, 0], 6);

              expect(
                autocorrelate(inputType.arrayConverter([1, 2, 3, 4]))
              ).toBeDeepCloseTo([30, 20, 11, 4], 6);

              expect(
                autocorrelate(
                  inputType.arrayConverter([1, -1, 1, -1, 1, -1, 1, -1])
                )
